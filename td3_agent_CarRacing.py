...
class CarRacingTD3Agent(TD3BaseAgent):
	...
	def decide_agent_actions(self, state, sigma=0.0, brake_rate=0.015):
		...
		with torch.no_grad():
			...
			# state 4,96,96
			state = torch.from_numpy(state).to(torch.float).to(device=self.device)
			action = self.actor_net(state).squeeze().cpu().numpy() + sigma * self.noise.generate()
	...
	def update_behavior_network(self):
		...
		with torch.no_grad():
			...
			# TODO clip noise
			# print("next_state",next_state.shape) 32,4,96,96
			noise = self.noise.generate()
			noise = torch.from_numpy(noise).to(torch.float).to(device=self.device)

			a_next = self.target_actor_net(next_state, target = True) + noise
		...
		if self.total_time_step % self.update_freq == 0:
			...
			action = self.actor_net(state,target = True)
			actor_loss = -1 * (torch.mean(self.critic_net1(state, action)))
			# optimize actor
